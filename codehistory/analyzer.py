...
class EvolutionAnalyzer:
    ...
    def analyze(
        ...
        if previous is None:
            ...
            events.append(
                EvolutionEvent(
                    event_type="BORN",
                    detail={
                        "call_tree_nodes": current.call_tree_nodes,
        ...
        if node_ratio >= self.growth_threshold:
            events.append(
                EvolutionEvent(
                    event_type="GROWN",
                    detail={
                        "from_nodes": previous.call_tree_nodes,
        ...
        if depth_ratio >= self.growth_threshold:
            events.append(
                EvolutionEvent(
                    event_type="EXTENDED",
                    detail={
                        "from_depth": previous.call_tree_depth,
        ...
        if current.call_tree_edges > previous.call_tree_edges:
            events.append(
                EvolutionEvent(
                    ...
                    detail={
                        "from_edges": previous.call_tree_edges,
        ...
        if previous.file_path != current.file_path:
            events.append(
                EvolutionEvent(
                    event_type="MOVED",
                    detail={"from_file": previous.file_path, "to_file": current.file_path},
                )
            )
            changed = True
        ...
        if current.test_nodes > previous.test_nodes:
            events.append(
                EvolutionEvent(
                    ...
                    detail={
                        "from_test_nodes": previous.test_nodes,
    ...
    def died_event(self, last_snapshot: SnapshotData) -> EvolutionEvent:
        ...
        return EvolutionEvent(
            event_type="DIED",
            detail={
                "last_call_tree_nodes": last_snapshot.call_tree_nodes,
