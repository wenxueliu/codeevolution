...
class AdvancedTopologyImplementation:
    ...
    def trace_full_flow(
        ...
        while queue:
            ...
            # B4: terminate flow branches at databases used by this service.
            for access in database_edges.get(svc, []):
                key = (svc, f"db:{access['target']}", access["table"])
                if key in visited_pairs:
                    continue
                visited_pairs.add(key)
                channels_used.add("db")
                steps.append(
                    FlowStep(
                        depth=depth,
                        channel="db",
                        from_service=svc,
                        from_function=access["function"],
    ...
    def _collect_http_edges(self) -> dict[str, list[dict]]:
        ...
        analyzer = CrossRepoImplementation(self.repos)
    ...
    def _collect_db_edges(self) -> dict[str, list[dict]]:
