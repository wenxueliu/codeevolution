...
# CodeEvolution Knowledge Extraction
...
python -m codeevolution.cli register -n <service-name> -r /path/to/repo
python -m codeevolution.cli discover -d /path/to/repos   # scan directory
python -m codeevolution.cli check                         # health check
```
...
python -m codeevolution.cli knowledge -r <repo_path> -s api       # API contract
python -m codeevolution.cli knowledge -r <repo_path> -s modules    # Module topology
python -m codeevolution.cli knowledge -r <repo_path> -s entities   # Core entities (PageRank)
python -m codeevolution.cli knowledge -r <repo_path> -s tests      # Test coverage gaps
python -m codeevolution.cli knowledge -r <repo_path> -s layers     # Layer violations

# Phase 2 — Graph + rules (fast, no LLM)
python -m codeevolution.cli knowledge -r <repo_path> -s config     # Config consumption
python -m codeevolution.cli knowledge -r <repo_path> -s deps       # External dependencies
python -m codeevolution.cli knowledge -r <repo_path> -s auth       # Authorization model
python -m codeevolution.cli knowledge -r <repo_path> -s heatmap    # Heat map (hot/warm/cold)

# Phase 3 — LLM-powered (requires OPENAI_API_KEY)
python -m codeevolution.cli knowledge -r <repo_path> -s business --llm   # Business descriptions
python -m codeevolution.cli knowledge -r <repo_path> -s rules --llm      # Business rules
python -m codeevolution.cli knowledge -r <repo_path> -s errors --llm     # Error catalog
python -m codeevolution.cli knowledge -r <repo_path> -s states --llm     # State machines

# Export
python -m codeevolution.cli knowledge -r <repo_path> -o report.json        # Phase 1+2
python -m codeevolution.cli knowledge -r <repo_path> -o report.json --llm  # Full
```
...
# Service management
python -m codeevolution.cli register -n <name> -r /path/to/repo   # Register with auto-detection
python -m codeevolution.cli discover -d /path/to/repos              # Scan for git repos
python -m codeevolution.cli check                                   # Health check all services

# Topology (cached after first build)
python -m codeevolution.cli topology                                # Unified service graph
python -m codeevolution.cli impact -s <service>                     # Cross-service change impact
python -m codeevolution.cli trace -s <service>                      # HTTP call chain trace
python -m codeevolution.cli flow -s <service>                       # Full flow (HTTP+MQ+gRPC+DB)
python -m codeevolution.cli entities [--llm]                        # Cross-service entity alignment
```
